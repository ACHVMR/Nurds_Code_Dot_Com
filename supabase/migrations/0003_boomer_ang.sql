...
CREATE TABLE boomer_angs (
  ...
  image_url TEXT, -- Stored in R2/Cloudflare
  
  -- Categorization
  category VARCHAR(100) NOT NULL DEFAULT 'General',
  effectiveness_level VARCHAR(50) NOT NULL DEFAULT 'Basic', -- Basic, Advanced, Premium, Enterprise
  ...
  -- Configuration
  config JSONB DEFAULT '{}'::jsonb, -- Model, temperature, system prompt, etc.
  
  -- Ownership
  creator_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
  is_premade BOOLEAN DEFAULT false, -- Official Nurds Code creation
  is_public BOOLEAN DEFAULT false, -- Available in marketplace
  
  -- Marketplace
  price DECIMAL(10, 2) DEFAULT 0, -- Purchase price (0 = free)
  rent_price DECIMAL(10, 2) DEFAULT 0, -- Monthly rental price
  
  -- Stats
  success_rate DECIMAL(5, 2) DEFAULT 0, -- Percentage
  total_runs INTEGER DEFAULT 0,
  tokens_per_run INTEGER DEFAULT 0, -- Average
...
CREATE TABLE user_boomer_angs (
  ...
  -- Ownership Type
  ownership_type VARCHAR(50) NOT NULL, -- owned, rented, sandbox
...
CREATE TABLE marketplace_transactions (
  ...
  -- Transaction Type
  transaction_type VARCHAR(50) NOT NULL, -- purchase, rental_start, rental_end
...
CREATE TABLE sandbox_sessions (
  ...
  -- Limits
  max_runs INTEGER DEFAULT 100, -- Limit for sandbox testing
...
-- Update Boomer_Ang rating when new rating is added
...
-- Increment run count when run completes
...
-- Expire rental when rental_end is reached
...
-- View for user's complete agent collection
...
-- Example: Expire rentals every hour
-- SELECT cron.schedule('expire-rentals', '0 * * * *', 'SELECT expire_rentals()');

-- Example: Expire sandbox sessions daily
-- SELECT cron.schedule('expire-sandbox', '0 0 * * *', 'SELECT expire_sandbox_sessions()');
